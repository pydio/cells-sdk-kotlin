...
enum class ActivityStreamContext(val value: kotlin.String) {
  ...
  @Json(name = "MYFEED") MYFEED("MYFEED"),
  @Json(name = "USER_ID") USER_ID("USER_ID"),
  @Json(name = "NODE_ID") NODE_ID("NODE_ID");
  ...
  companion object {
    ...
    fun encode(data: kotlin.Any?): kotlin.String? =
        if (data is ActivityStreamContext) "$data" else null

    /** Returns a valid [ActivityStreamContext] for [data], null otherwise. */
    fun decode(data: kotlin.Any?): ActivityStreamContext? =
        data?.let {
